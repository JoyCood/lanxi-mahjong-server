...
func (this *IappTrans) IpaySign(tran []byte) ([]byte, error) {
	return this.Config.Cipher.Sign(tran)
}

// RSA验签
func (this *IappTrans) IpayVerify(tran []byte, sign []byte) error {
	return this.Config.Cipher.Verify(tran, sign)
}

// 下单
func (this *IappTrans) Submit(price uint32, orderId, uid, itemid string) (string, error) {
	iappOdr := this.newIappayOrder(price, orderId, uid, itemid)
...
// 下单请求参数
func (this *IappTrans) newIappayOrder(price uint32, id, uid, itemid string) *IapppayOrder {
	return &IapppayOrder{
		Appid:         this.Config.AppId,     //应用编号
		Waresid:       1,              //商品编号
		Waresname:     "diamond",      //商品名称
		Cporderid:     id,             //商户订单号
